    ...
    public void run(
        ...
        try (Connection connection = DriverManager.getConnection(jdbcConnectionString);
             ...
             ResultSet resultSet = statement.executeQuery(
                     "SELECT MIN(temperature), MAX(temperature), AVG(CAST(temperature AS FLOAT)), " +
                             "MIN(wind_speed), MAX(wind_speed), AVG(CAST(wind_speed AS FLOAT)), " +
                             "MIN(humidity), MAX(humidity), AVG(CAST(humidity AS FLOAT)), " +
                             "MIN(co2_level), MAX(co2_level), AVG(CAST(co2_level AS FLOAT)) " +
                             "FROM SensorData")) {
