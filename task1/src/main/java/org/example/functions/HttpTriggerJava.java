...
public class HttpTriggerJava {
    ...
    public HttpResponseMessage run(
        ...
        context.getLogger().info("HTTP trigger executed at: " + java.time.LocalDateTime.now());
        ...
        for (int i = 1; i <= 20; i++) {
            ...
            try (Connection connection = DriverManager.getConnection(JDBCconnectString);
                ...
                return request.createResponseBuilder(HttpStatus.INTERNAL_SERVER_ERROR).body("Error inserting data: " + e.getMessage()).build();
            }
        }

        return request.createResponseBuilder(HttpStatus.OK).body("Data inserted successfully").build();
    }
}